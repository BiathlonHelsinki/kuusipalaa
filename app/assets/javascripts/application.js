...
function check_additionals() {
  ...
  idarray.forEach(function (eyed) {
    var start = moment(
      $('input[name="idea[additionaltimes_attributes][' + eyed + '][start_at_date]"]').val() +
        ' ' +
        $('input[name="idea[additionaltimes_attributes][' + eyed + '][start_at]"]').val(),
      'YYYY-MM-DD HH:mm'
    )
    var endtime = moment(
      $('input[name="idea[additionaltimes_attributes][' + eyed + '][end_at_date]"]').val() +
        ' ' +
        $('input[name="idea[additionaltimes_attributes][' + eyed + '][end_at]"]').val(),
      'YYYY-MM-DD HH:mm'
    )
    if (endtime.isBefore(start)) {
      ...
      $('#slotlength_' + eyed).html(
        endtime.preciseDiff(start) + '<Br />+' + parseInt(base) + ' points needed'
      )
      returned.push(base)
    }
  })
  ...
  idarray.forEach(function (eyed) {
    ...
    if (endtime.isBefore(start)) {
      array_of_functions.push(() => {
        $('input[name="idea[additionaltimes_attributes][' + eyed + '][end_at]"]').val(
          correctHour(
            $('input[name="idea[additionaltimes_attributes][' + eyed + '][start_at]"]')
              .val()
              .match(/\d\d/)[0]
          ) + ':00'
        )
...
function calculatePrivate() {
  ...
  var array_of_functions = []
  var start = moment(
    $('#roombooking_start_at_date').val() + ' ' + $('#roombooking_start_at').val(),
    'YYYY-MM-DD HH:mm'
  )
  var endtime = moment(
    $('#roombooking_end_at_date').val() + ' ' + $('#roombooking_end_at').val(),
    'YYYY-MM-DD HH:mm'
  )

class Pledge < ApplicationRecord
  ...
  def check_balance
    ...
    return unless pledge < 0 || pledge > pledger.latest_balance
    errors.add(:pledge, 'You cannot pledge this amount.')
  end

  def one_per_user
    return if item.pledges.where(pledger: pledger, converted: false).to_a.delete_if { |x| x == self }.empty?
  ...
  def update_activity_feed
    ...
    return unless item.instance_of?(Proposal)
    item.update_column_caches
    item.save
  end

  def withdraw_activity
    item.comments << Comment.create(user: user, contributor: pledger, content: "Pledge of #{pledge}#{ENV['currency_symbol']} withdrawn.", systemflag: true)

  ...
  def publish_event
    ...
    if @idea.converted?
      ...
      if @idea.has_enough? && (@idea.proposers.include?(current_user) || current_user.has_role?(:admin))
        @event = Event.new(idea: @idea, place_id: 2, primary_sponsor: @idea.proposer, remote_image_url: @idea.image? ? @idea.image.url.gsub(/development/, 'production') : nil, cost_euros: @idea.price_public, cost_bb: @idea.price_stakeholders, 

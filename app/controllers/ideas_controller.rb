class IdeasController < ApplicationController
  ...
  def calendar
    ...
    (start..enddate).each do |day|
      ...
      @times << { title: t(:choose) + l(day.to_date, format: :short), day: day, start: "#{day} 00:00:01".to_s, end: "#{day} 23:59:59".to_s, allDay: true, recurring: false, url: "choose_day/#{day.to_fs(:db)}" }
    end
  ...
  def edit
    @idea = Idea.friendly.find(params[:id])
    return unless can? :edit, @idea
    @idea.update_attribute(:status, 'name_and_info')
    flash[:notice] = t(:you_are_editing_your_proposal)
    return unless @idea.pledges.empty?
    redirect_to("/ideas/#{@idea.id}/build/name_and_info")
  end
  ...
  def archive
    @ideas = Idea.all.where(status: %w[active converted cancelled]).order(updated_at: :desc)
  end
  # def new
  ...
  def publish_event
    ...
    elsif @idea.has_enough? && (@idea.proposers.include?(current_user) || current_user.has_role?(:admin))
  ...
  def show
    ...
    if @idea.converted?
      ...
      if user_signed_in?
        ...
        @pledge = if current_user.pledges.unconverted.where(item: @idea).empty?
          @idea.pledges.build
        else
          ...
          current_user.pledges.unconverted.find_by(item: @idea)
        end
      end
  ...
  def update # should only happen if cancelled or edited
    ...
    if params[:idea][:additionaltimes_attributes]
      params[:idea][:additionaltimes_attributes].permit!.to_hash.each_with_index do |pa, _index|
    ...
    return unless @idea.update_attributes(idea_params)
  ...
  def idea_params
    params.require(:idea).permit([:ideatype_id, :status, :form_direction, :timeslot_undetermined, :start_at, :end_at, :price_public, :room_needed, :price_stakeholders, :allow_others, :name, :short_description, :proposal_text,
                                  ...
                                  :points_needed, { additionaltimes_attributes: %i[id _destroy start_at end_at] }])
  end
end
